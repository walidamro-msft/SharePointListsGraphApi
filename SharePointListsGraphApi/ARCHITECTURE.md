...
User selects export
    ?
User prompted: Filter by Modified date? (y/n)
    ?
If yes:
    Display default dates (today 00:00:00 to 23:59:59)
    ?
    User input: From date (or press Enter for default)
    ?
    User input: To date (or press Enter for default)
    ?
ListExportService.DownloadAllListItemsAsync(fromDate, toDate)
    ?
Build OData filter query if dates provided
    ?
GraphClient.Sites[].Lists[].Items.GetAsync()
    Apply filter: fields/Modified ge 'fromDate' and fields/Modified le 'toDate'
    ?
While has pages:
