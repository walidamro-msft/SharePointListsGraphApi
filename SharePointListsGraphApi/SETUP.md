...
- **Expires**: Choose expiration period (e.g., 24 months)
  - ⚠️ **Important**: Note the expiration date - you'll need to renew before it expires

4. Click **Add**
5. **Copy the Value immediately** - you cannot retrieve it later!
   - This is your `ClientSecret` for `appsettings.json`

⚠️ **Security Note**: Store this secret securely. Never commit it to source control.
...
| Sites.ReadWrite.All | Application | Yes |
| Sites.Selected | Application | Yes |
...
2. **⚠️ IMPORTANT**: Set `appsettings.json` to **Copy to Output Directory**:
   - In Visual Studio: Right-click `appsettings.json` → Properties → **Copy to Output Directory**: "Copy if newer"
   - Or add this to your `.csproj` file:
     ...
     ```
   - **Without this step, the application will fail to find appsettings.json at runtime!**
...
4. Update the SharePoint-specific values:
