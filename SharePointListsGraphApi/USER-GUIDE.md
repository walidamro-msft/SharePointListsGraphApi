...
2. **Date Filtering (Optional)**
   - The tool will ask if you want to filter by the Modified date
   - You can filter to export only items modified within a specific date range
   
   ```
   Do you want to filter by Modified date? (y/n):
   ```
   
   **Option A - Apply Filter (Answer "y"):**
   ```
   Default date range (press Enter to accept, or type new values):
     From: 2024-12-15 00:00:00
     To:   2024-12-15 23:59:59
   
   From date and time [default: 2024-12-15 00:00:00]: 
   To date and time [default: 2024-12-15 23:59:59]: 
   ```
   
   - **Press Enter** to use today's date range (00:00:00 to 23:59:59)
   - **Or type a custom date** in format: `yyyy-MM-dd HH:mm:ss`
   - Examples:
     - `2024-01-01 00:00:00` (specific date at midnight)
     - `2024-12-01 09:30:00` (specific date and time)
   
   **Option B - No Filter (Answer "n"):**
   - Downloads all items from the list regardless of modification date

3. **Automatic Export**
   - The tool exports based on your date filter (if applied)
...
4. **Progress Display**
   ...
   Processing page 1 with 50 items... Total: 50
   Processing page 2 with 30 items... Total: 80
   
   ? Total items downloaded: 80
   ? Successfully saved 80 items to 20241215143022.json
   ```

### Date Filter Details

**Modified Field:**
- Filters based on SharePoint's built-in "Modified" metadata field
- This field is automatically updated whenever an item changes
- Includes both the date and time of modification

**Default Behavior:**
- **From Date**: Today at 00:00:00 (start of day)
- **To Date**: Today at 23:59:59 (end of day)
- This exports all items modified today

**Use Cases:**
- **Daily exports**: Accept defaults to get today's changes
- **Date range**: Enter custom dates to export a specific period
- **Recent changes**: Filter last week/month of modifications
- **Full backup**: Answer "n" to skip filtering and get all items
...
- Use `*` to export all columns (including system fields like Modified, Created, etc.)
...
- **Date filter only applies at export time** - not stored in appsettings.json
- Press Enter to quickly accept today's date range for daily exports
- Use custom dates for historical data or specific time periods
